...
if [[ "$action" == "link" ]]; then
    cur_ses=$(tmux display-message -p | /nix/store/ql68miwsgz9094bi3qa7nk17bfwf6h6a-gnused-4.9/bin/sed -e 's/^.//' -e 's/].*//')
    last_win_num=$(tmux list-windows | sort -r | /nix/store/ql68miwsgz9094bi3qa7nk17bfwf6h6a-gnused-4.9/bin/sed '2,$d' | /nix/store/ql68miwsgz9094bi3qa7nk17bfwf6h6a-gnused-4.9/bin/sed 's/:.*//')
    ...
    src_win=$(echo "$src_win_origin" | /nix/store/ql68miwsgz9094bi3qa7nk17bfwf6h6a-gnused-4.9/bin/sed 's/: .*//')
    ...
    if [[ "$action" != "switch" ]]; then
        ...
        target_origin=$(echo "$target_origin" | /nix/store/ql68miwsgz9094bi3qa7nk17bfwf6h6a-gnused-4.9/bin/sed -E "s/\[current\]/$current_window_origin/")
    else
        windows=$(echo "$windows" | grep -v "^$current_window")
        target_origin=$(printf "%s\n[cancel]" "$windows" | eval "$TMUX_FZF_BIN $TMUX_FZF_OPTIONS $TMUX_FZF_PREVIEW_OPTIONS")
    fi
    [[ "$target_origin" == "[cancel]" || -z "$target_origin" ]] && exit
    target=$(echo "$target_origin" | /nix/store/ql68miwsgz9094bi3qa7nk17bfwf6h6a-gnused-4.9/bin/sed 's/: .*//')
    if [[ "$action" == "kill" ]]; then
        ...
        target_swap=$(echo "$target_swap_origin" | /nix/store/ql68miwsgz9094bi3qa7nk17bfwf6h6a-gnused-4.9/bin/sed 's/: .*//')
        tmux swap-pane -s "$target" -t "$target_swap"
    elif [[ "$action" == "switch" ]]; then
        echo "$target" | /nix/store/ql68miwsgz9094bi3qa7nk17bfwf6h6a-gnused-4.9/bin/sed 's/:.*//g' | xargs -I{} tmux switch-client -t {}
