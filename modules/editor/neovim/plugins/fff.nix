{
  ...
  extraConfigLue = ''
    -- map("n", "<C-f>", ":FFFFind <CR>")
    -- map("n", "<C-f>", ":FFFFind /home/lunics <CR>")
    ...
    require('fff').setup {
      ...
      keymaps = {
        ...
        move_up             = { '<C-k>', '<Up>' },
        move_down           = { '<C-j>', '<Down>' },
        preview_scroll_up   = '<C-S-j>',
        preview_scroll_down = '<C-S-k>',
        toggle_debug        = '<F2>',
      ...
      debug = {
        enabled        = true,   -- show scores in the UI
        show_scores    = true,
        show_file_info = true,   -- show file info panel in preview
      },
      logging = {
        enabled   = true,
