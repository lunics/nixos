{ inputs, ... }:{
  ...
  flake.aspects.audio.nixos = { config, lib, pkgs, ... }:{
    ...
    config = lib.mkIf config._.musnix {
      musnix = {
        # list options: nixos-option musnix.rtirq or nixos-option musnix.rtirq.enable
        enable         = true;
        alsaSeq.enable = true;   # load ALSA sequencer kernel modules
        ffado.enable   = false;  # use the Free FireWire Audio Drivers (FFADO)
        rtcqs.enable   = false;  # install the rtcqs command-line utility
        ...
        # das_watchdog.enable = false;      # start the das_watchdog service to ensure that a realtime process won't hang the machine
      };
