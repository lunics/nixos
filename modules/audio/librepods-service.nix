{
  flake.aspects = {
    options.generic = { lib, ... }: with lib; {
      options._.airpods_mac = mkOption {
        type    = types.nullOr types.str;
        default = null;      # "AA:BB:CC:DD:EE:FF", MAC address of the AirPods driving librepods
      };
    };

    librepods.homeManager = { config, lib, pkgs, ... }: with lib;
    let
      mac = config._.airpods_mac;
      dev = "dev_" + toUpper (builtins.replaceStrings [ ":" ] [ "_" ] (toString mac));
